<div>
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6 space-y-4">
            {{-- Alerta: Próximas Cobranzas --}}
            @if($this->stats['proximas_cobrar'] > 0)
                <div class="bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-500 p-5 rounded-xl shadow-lg">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4 flex-1">
                            <h3 class="text-xl font-bold text-yellow-900">
                                💰 ¡RECORDATORIO! Tienes {{ $this->stats['proximas_cobrar'] }} cobranza(s) próxima(s)
                            </h3>
                            <p class="text-sm text-yellow-800 mt-1">
                                Hay pólizas con fechas de cobro en los próximos 7 días - ¡No olvides contactar a tus clientes!
                            </p>
                        </div>
                        <div class="ml-auto">
                            <a href="#polizas-cobrar" 
            ...
            {{-- Alerta: Pólizas por Vencer --}}
            @if($this->stats['proximas_vencer'] > 0)
                <div class="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 p-5 rounded-xl shadow-lg">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                        ...
                        <div class="ml-auto">
                            <a href="#polizas-vencer" 
                               class="inline-flex items-center px-5 py-2 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-lg transition shadow-md">
                                Ver detalles
                            </a>
                        </div>
                    </div>
        ...
        {{-- NUEVO: Próximas Cobranzas --}}
        @if($this->stats['proximas_cobrar'] > 0)
            <div id="polizas-cobrar" class="bg-white shadow-xl rounded-xl p-6 mb-6 border border-gray-100">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                            <span class="text-3xl">💰</span>
                            Próximas Cobranzas (7 días)
                        </h3>
                        <p class="text-sm text-gray-600 mt-1">
                            Pólizas con fechas de cobro próximas - ¡Recuerda contactar a tus clientes!
                        </p>
                    </div>
                    <div>
                        <span class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-bold rounded-xl shadow-lg text-lg animate-pulse">
                            {{ $this->stats['proximas_cobrar'] }} cobranzas
                        </span>
                    </div>
                </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-gradient-to-r from-blue-50 to-indigo-50 border-b-2 border-blue-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Compañía</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Forma Pago</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Fecha Cobro</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($this->polizasProximasCobrar as $poliza)
                                @php
                                    ...
                                    $urgencia = $dias <= 1 ? 'critico' : ($dias <= 3 ? 'urgente' : 'proximo');
                                    $badgeClass = [
                                        'critico' => 'bg-red-100 text-red-800 border-2 border-red-400 animate-pulse',
                                        'urgente' => 'bg-orange-100 text-orange-800 border-2 border-orange-400',
                                        'proximo' => 'bg-yellow-100 text-yellow-800 border-2 border-yellow-400'
                                    ][$urgencia];
                                @endphp
                                <tr class="hover:bg-blue-50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">
                                         {{ \Carbon\Carbon::parse($poliza->proxima_fecha_cobranza)->format('d/m/Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-600">
                                        ${{ number_format($poliza->proximo_monto_cobro, 2) }}
                                    </td>
        ...
        @if($this->stats['proximas_cobrar'] == 0 && $this->stats['proximas_vencer'] == 0)
            <div class="bg-white shadow-xl rounded-xl p-12 mb-6 border border-gray-100 text-center">
                <svg class="mx-auto h-24 w-24 text-green-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <h3 class="text-3xl font-bold text-gray-900 mb-2">¡Todo al día!</h3>
                <p class="text-lg text-gray-600">
                    No hay pólizas próximas a vencer ni cobranzas pendientes en los próximos días. ¡Excelente trabajo! 🎉
                </p>
            </div>
        @endif
                        ...
                        <div>
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex-1">
                                    ...
                                    <span class="text-xs font-medium text-gray-600 block mt-0.5">
                                        {{ $compania->Cobertura }}
                                    </span>
                                </div>
